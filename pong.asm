...
jmp start			; This should always be before any include!!!
...
start:
	...
	mov ds, ax              ; Setup datasegment
        add ax, 020h		; The size of the bootloader is 512 bytes.
				...
				; So we add (512/16) to it, to go after our
 				; bootloader.
	mov ss, ax
...
.main_loop:
	...
	; 33.333 = 0b1000001000110101 Luckily this number fits a 16-bit
	; register.
	mov ah, 86h
	mov cx, 0
	mov dx, 12500
...
.player_line:
	mov ax, [player_x]	; Check if player_x <= ball_x <= player_x +
	cmp [ball_x], ax	; player_width
	jb .enemy_point		; If yes, flip the y_step of the ball.
	add ax, PLAYER_WIDTH	; Else give the enemy a point.
	cmp [ball_x], ax
	ja .enemy_point
	; BALL HIT PLAYER
	neg word [ball_step_y]
